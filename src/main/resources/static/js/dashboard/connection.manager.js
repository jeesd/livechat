    ...
    var CometBinding = function(cometd, liveCommunicator) {
        ...
        cometd.addListener('/meta/handshake', function (handshake){
            if (handshake.successful === true) {
                ...
                cometd.batch(function () {
                    cometd.subscribe('/dashboard', function (message) {
                        liveCommunicator._emit('timeout', message, message.id);
                    });
                    cometd.subscribe('/visitor/'+handshake.ext.accountId, function (message) {
                        liveCommunicator._emit(message.data.type, message.data, message.data.id);
                    });

                    // Publish on a service channel since the message is for the server only
                    //cometd.publish('/dashboard/app', { name: 'World' });
                });
                console.log(handshake);
            }else if (handshake.successful === false && handshake.com.livedata.auth != null  && handshake.com.livedata.auth == 'not_available_connections') {
                cometd.disconnect();
            }
        ...
        cometd.addListener('/meta/connect', function (message) {
            ...
            if (!wasConnected && liveCommunicator.connected) {
                ...
                liveCommunicator._emit('OPERATORS_COUNT', message.ext.onlineUsersCount, util.guid());
            }
